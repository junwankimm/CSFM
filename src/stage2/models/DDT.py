...
class VectorEmbedder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, dropout_prob=0.0) -> torch.Tensor:
        is_null = (x.abs().sum(dim=1, keepdim=True) == 0) #  TODO: assuming 0 null tokens
        y_embed = self.mlp(x)
        if self.training and dropout_prob > 0.0:
            B = y_embed.size(0)
            drop_mask = (torch.rand(B, device=x.device) < dropout_prob)
            mask_y = drop_mask.view(B, *([1] * (y_embed.ndim - 1)))
            y_embed = torch.where(mask_y, torch.zeros_like(y_embed), y_embed)
        elif not self.training:
            y_embed = torch.where(is_null, torch.zeros_like(y_embed), y_embed)
...
class DiTwDDTHead(nn.Module):
    def __init__(
            ...
            mlp_ratio=4.0,
            y_dropout_prob=0.1,
            class_dropout_prob=0.1, # legacy for previous configs
            use_y_for_adaln=False,
            y_in_channels=768,
            use_qknorm=False,
            use_swiglu=True,
            use_rope=True,
        ...
        self.use_y_for_adaln = use_y_for_adaln
        ...
        s_channel_per_token = in_channels
        ...
        x_channel_per_token = in_channels
        ...
        self.y_embedder = VectorEmbedder(y_in_channels, self.encoder_hidden_size)
    ...
    def forward(self, 
        ...
        **kwargs
        ...
        assert mask is None, "Mask not supported for DiTwDDTHead."
        assert s is None, "s input not supported for DiTwDDTHead."

        # x = self.x_embedder(x) + self.pos_embed
        t = self.t_embedder(t)
        if y is not None and self.use_y_for_adaln:
            y = self.y_embedder(y, dropout_prob=self.y_dropout_prob)
    ...
    # Will not use for our setting. CFG should be different. 
    def forward_with_cfg(self, x, t, y=None, cfg_scale=1.0, cfg_interval=(0, 1), **kwargs):
        """
    ...
    # Autoguidance should be changed too
    def forward_with_autoguidance(self, x, t, y=None, cfg_scale=None, additional_model_forward=None, cfg_interval=(0, 1)):
        """
