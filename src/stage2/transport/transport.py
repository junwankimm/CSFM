...
from .integrators import ode, sde, ode_npe
...
class Transport:
    ...
    def training_losses(
        ...
        """Loss for training the representation to representation transport model
        Args:
        - model: backbone model; could be score, noise, or velocity
        - x1: datapoint
        - y: input condition (clip embedding)
        - model_kwargs: additional arguments for the model
        """
        if model_kwargs == None:
            model_kwargs = {}
        
        clip_latents, padding_masks, pooler_output = y
        t = self.sample_timestep(x1)
        
        logit_scale, (x0, mu, log_var) = wrapper_model(x=clip_latents, padding_mask=padding_masks, textve=True)
        
        apply_noise = ["cosine_similarity", "rkd"]
        if self.regularization_loss in apply_noise and log_var is None:
            std = th.full_like(x0, self.std)
            noise = th.randn_like(x0)
            x0 = x0 + std * noise
            
        # Currently using mean pooling for contrastive loss
        anchor = x0.mean(dim=1)
        std = x0.std(dim=1).mean()
        norm = x0.view(x0.size(0), -1).norm(dim=1)
        
        target = pooler_output
        if self.contrastive_loss is not None:
            anchor = anchor / anchor.norm(dim=-1, keepdim=True)
            target = target / target.norm(dim=-1, keepdim=True)
            contrastive_loss = self.contrastive_loss(anchor, target, logit_scale=logit_scale.exp())
        else:
            print("Using cosine similarity as contrastive loss")
        
        x0 = x0.permute(0, 2, 1).contiguous().view(x1.shape)
        
        if self.regularization_loss == "kld":
            kld_loss = -0.5 * th.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1) # (Real KLD)
        elif self.regularization_loss == "naive_kld":
            kld_loss = -0.5 * th.sum(1 + log_var - (0.3 * mu) ** 6 - log_var.exp(), dim = -1) # (Crossflow KLD for stability in small dataset)
        elif self.regularization_loss == "cosine_similarity":
            kld_loss = token_cosine_loss(x0, x1)
        elif self.regularization_loss == "rkd":
            kld_loss = rkd(x0, x1, pool_kernel=4)
        else:
            raise NotImplementedError(f"Unknown regularization loss {self.regularization_loss}")

        t, xt, ut = self.path_sampler.plan(t, x0, x1)
        model_output = wrapper_model(x=xt, t=t, textve=False, **model_kwargs)
        B, *_, C = xt.shape
        ...
        if self.model_type == ModelType.VELOCITY:
            if detach_ut:
                terms['diffusion_loss'] = mean_flat(((model_output - ut.detach()) ** 2))
            else:
                terms['diffusion_loss'] = mean_flat(((model_output - ut) ** 2))
            terms['contrastive_loss'] = contrastive_loss
            terms['kld_loss'] = mean_flat(kld_loss)
            terms['logit_scale'] = logit_scale
            terms['std'] = std
            terms['norm'] = norm
        else: 
            ### Nor handled yet
            ...
            if self.model_type == ModelType.NOISE:
                terms['diffusion_loss'] = mean_flat(weight * ((model_output - x0) ** 2))
            else:
                terms['diffusion_loss'] = mean_flat(weight * ((model_output * sigma_t + x0) ** 2))
                
        return terms
    
    def training_losses_textve(
        self, 
        wrapper_model,  
        x1, 
        y,
        ...
        if self.regularization_loss == "kld":
            ...
            kld_loss = rkd(x0, x1, pool_kernel=2)
        else:
            raise NotImplementedError(f"Unknown regularization loss {self.regularization_loss}")
        
        terms = {}
        terms['pred'] = x0
        terms['diffusion_loss'] = mean_flat(((x0 - x1) ** 2))
        terms['contrastive_loss'] = contrastive_loss
        terms['kld_loss'] = mean_flat(kld_loss)
        terms['logit_scale'] = logit_scale
        terms['std'] = std
...
class Sampler:
    ...
    def sample_ode_npe(
        ...
        *,
        ...
        return _ode.sample_with_energy