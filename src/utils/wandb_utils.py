...
def initialize(args, entity, exp_name, project_name, **kwargs):
    config_yaml = kwargs.get("config_yaml", None)
    config_dict = namespace_to_dict(args)
    
    if config_yaml is not None:
        config_yaml_dict = OmegaConf.to_container(config_yaml, resolve=True)
        config_dict = {
            "train_config": config_yaml_dict,
            "args": config_dict
        }
    ...
    wandb.init(
        ...
        name=exp_name,
        config=config_dict,
        # id=generate_run_id(exp_name), # different run even with same exp_name
        resume="allow",
    )
