...
class CoordPatchDecoder(nn.Module):
    def __init__(self, p, dec_dim, out_channels, d=128, pe_dim=64):
        super().__init__()
        self.p = p
        ...
        self.row = nn.Linear(dec_dim, p * d)
        self.col = nn.Linear(dec_dim, p * d)
        self.hproj = nn.Linear(dec_dim, d)

        self.mlp = nn.Sequential(
            nn.Linear(d + pe_dim, d),
            nn.GELU(),
    ...
    def forward(self, h):
        ...
        Hproj = self.hproj(h)             # (B, N, d)
        Row = self.row(h).view(B, N, p, self.d)
        Col = self.col(h).view(B, N, p, self.d)

        # Build PE table for all i,j
        pe_table = []
        for i in range(p):
            for j in range(p):
                pe_table.append(fourier_pe(i, j, p, self.pe_dim))
        pe_table = torch.stack(pe_table, dim=0) # (p*p, pe_dim)
        pe_table = pe_table.to(h.device)
        ...
        for idx in range(p*p):
            i = idx // p
            j = idx % p
            z = Hproj + Row[:,:,i,:] + Col[:,:,j,:]     # (B,N,d)
            z = torch.cat([z, pe_table[idx].unsqueeze(0).unsqueeze(0).expand(B,N,-1)], dim=-1)
            pix = self.mlp(z)   # (B,N,3)
            outputs.append(pix)

        # stack: (B,N,p*p,3) → reshape for unpatchify
        out = torch.stack(outputs, dim=2)
        out = out.view(B, N, p*p*3)
        return out
    
class VectorizedCoordPatchDecoder(nn.Module):
    def __init__(self, p, dec_dim, out_channels, d=128, pe_dim=64):
        super().__init__()
        self.p = p
        self.d = d
        
        # 1. Dynamic Feature Generators
        # Hproj: Global context per token
        self.hproj = nn.Linear(dec_dim, d)
        # Row/Col: Spatial Decomposition features
        self.row = nn.Linear(dec_dim, p * d)
        self.col = nn.Linear(dec_dim, p * d)

        # 2. Static Position Embedding (Learnable or Fourier)
        # 여기서는 학습 효율을 위해 Buffer로 등록 (p가 바뀔때마다 재계산 로직 필요하지만 일단 고정)
        self.register_buffer('pe_table', torch.randn(1, 1, p, p, pe_dim)) 
        # 실제로는 __init__ 말고 forward에서 p에 따라 생성하거나 interpolate 해야 함
    ...
    def forward(self, h, target_p=None):
        # h: (B, N, dec_dim)
        B, N, C = h.shape
        
        # Resolution handling
        p = target_p if target_p is not None else self.p
        
        # ---------------------------------------------------------
        # 1. Dynamic Features Projection
        # ---------------------------------------------------------
        H_global = self.hproj(h).view(B, N, 1, 1, self.d) # (B, N, 1, 1, d)
        
        # Row/Col Projection
        # 원본이 p * d 였으므로, 만약 target_p가 다르면 interpolate 필요
        # 여기서는 일단 학습된 p 그대로 사용한다고 가정
        
        # (B, N, p, d) -> (B, N, p, 1, d)
        R_feat = self.row(h).view(B, N, self.p, 1, self.d) 
        
        # (B, N, p, d) -> (B, N, 1, p, d)
        C_feat = self.col(h).view(B, N, 1, self.p, self.d)

        # ---------------------------------------------------------
        # 2. Feature Interpolation (Resolution Adaptive의 핵심)
        # ---------------------------------------------------------
        if p != self.p:
            # Feature map 자체를 보간 (Smooth한 확장 가능)
            # Reshape for grid_sample or interpolate: [B*N, d, p, 1]
            R_feat = R_feat.reshape(B*N, self.p, 1, self.d).permute(0, 3, 1, 2) # [BN, d, p, 1]
            R_feat = F.interpolate(R_feat, size=(p, 1), mode='bicubic')
            R_feat = R_feat.permute(0, 2, 3, 1).view(B, N, p, 1, self.d)
            
            C_feat = C_feat.reshape(B*N, 1, self.p, self.d).permute(0, 3, 1, 2) # [BN, d, 1, p]
            C_feat = F.interpolate(C_feat, size=(1, p), mode='bicubic')
            C_feat = C_feat.permute(0, 2, 3, 1).view(B, N, 1, p, self.d)

        # ---------------------------------------------------------
        # 3. Broadcasting Sum (The "Factorized" Logic)
        # ---------------------------------------------------------
        # [B,N,1,1,d] + [B,N,p,1,d] + [B,N,1,p,d]
        # 자동으로 [B, N, p, p, d] 로 확장되어 더해짐
        # Loop 없이 한방에 계산됨!
        z = H_global + R_feat + C_feat 

        # ---------------------------------------------------------
        # 4. Concat PE & MLP
        # ---------------------------------------------------------
        # PE Table 확장: [1, 1, p, p, pe_dim] -> [B, N, p, p, pe_dim]
        # 만약 p가 바뀌면 pe_table도 interpolate 해야함 (생략)
        pe = self.pe_table 
        if p != self.p:
             pe = F.interpolate(pe.permute(0,4,1,2,3).squeeze(0), size=(p,p), mode='bilinear').unsqueeze(0).permute(0,2,3,4,1)
             
        pe_expanded = pe.expand(B, N, -1, -1, -1)
        
        # Concat: [B, N, p, p, d+pe_dim]
        z_in = torch.cat([z, pe_expanded], dim=-1)
        
        # MLP: [B, N, p, p, out_channels]
        out_grid = self.mlp(z_in)
        
        # Reshape to (B, N, p*p*3)
        out = out_grid.view(B, N, p*p, -1)
...
class GeneralDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        decoder_pred_type = config.get("decoder_head_type", "default")
        
        if decoder_pred_type == "coord":
            self.decoder_pred = CoordPatchDecoder(
                p=config.patch_size,
                dec_dim=config.decoder_hidden_size,
