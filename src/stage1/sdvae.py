...
class FrozenAutoencoderKL(nn.Module):
    def __init__(self, ddconfig, embed_dim, pretrained_path, reshape_to_2d, scale_factor=0.18215, do_preprocess=False, do_shuffle=False, shuffle_factor=2):
        super().__init__()
        print(f'Create autoencoder with scale_factor={scale_factor}')
        self.encoder = Encoder(**ddconfig)
        self.decoder = Decoder(**ddconfig)
        assert reshape_to_2d == True, "Currently only support reshape_to_2d=True"
    ...
    def encode(self, x):
        ...
        if self.do_shuffle:
            z = torch.nn.functional.pixel_unshuffle(z, downscale_factor=self.shuffle_factor)
        return z

    def decode(self, z):
        if self.do_shuffle:
            z = torch.nn.functional.pixel_shuffle(z, upscale_factor=self.shuffle_factor)
